...
export function AnimatedGridPattern({
  ...
  const updateSquarePosition = useCallback(
    (squareId: number) => {
      setSquares((current) => {
        const item = current[squareId]
        if (!item) return current
        const next = current.slice()
        next[squareId] = { ...item, pos: getPos(), iteration: item.iteration + 1 }
        return next
      })
    },
    [getPos],
  ...
  return (
    <svg
      ...
      aria-hidden="true"
      className={cn(
        'pointer-events-none absolute inset-0 h-full w-full fill-primary/20 stroke-primary/15',
        className,
      )}
      {...props}
    >
